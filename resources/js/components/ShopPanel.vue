<template>
    <div class="shop">
        <div class="tabs">
            ...
            <button :class="{ on: tab === 'cash' }" type="button" @click="$emit('tab', 'cash')">Донат</button>
            <button :class="{ on: tab === 'log' }" type="button" @click="$emit('tab', 'log')">Лента</button>
        </div>
        ...
        <div v-if="tab === 'upgrades'" class="list">
            ...
            >
                ...
                <p>{{ item.unlocked ? item.blurb : 'Ещё рано — копи норм и открывай этапы.' }}</p>
                <button
                    type="button"
                    class="buy"
                ...
                >
                    <template v-if="item.maxed">Максимум. И так норм.</template>
                    <template v-else-if="!item.unlocked">Пока закрыто</template>
        ...
        <div v-else-if="tab === 'cash'" class="list">
            <p class="cash-note">Деньги не списываются. Так выглядят СБП, боевой пропуск и реклама VK в бесплатной казуалке.</p>
            <article v-for="pack in monetization" :key="pack.sku" class="card cash">
                ...
                <p>{{ pack.blurb }}</p>
                <p v-if="pack.sku === 'insurance'" class="meta">Полисов «Не горит»: {{ pack.meta.insurance }}</p>
                <button
                    type="button"
                    class="buy gold"
                    :disabled="pack.owned"
                ...
                >
                    {{ pack.owned ? 'Уже куплено' : pack.type === 'ad' ? 'Смотреть ролик' : 'Оплатить через СБП' }}
                </button>
            </article>
        </div>

        <div v-else class="list log">
            <p v-if="!log.length" class="cash-note">Пока тихо. Кликай — УК и тётя Люда сами найдут тебя.</p>
